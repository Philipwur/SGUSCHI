...
def ReadPoscar(
    ...
    """
    Read a VASP POSCAR/CONTCAR file and return positions, cell dimensions, and (optionally) velocities.

    The function:
    - Locates POSCAR (or CONTCAR if POSCAR doesn't exist) unless FileName is given.
    - Parses lattice vectors, element counts, and fractional positions (converting from Cartesian if needed).
    - Optionally parses velocities (raw values as given in the file; no unit conversion).
    - Returns DataFrames with standard columns.

    Parameters
    ----------
    WorkDir : str or pathlib.Path, optional
        Directory containing POSCAR or CONTCAR (defaults to current working directory if None).
    FileName : str or pathlib.Path, optional
        Explicit file path; overrides automatic POSCAR/CONTCAR search.
    GiveVelocities : bool, optional
        If True, attempt to read and return atomic velocities.

    Returns
    -------
    (Position, CellDim) or (Position, CellDim, Velocities)
        Position : pandas.DataFrame
            Shape (N, 4) with columns ['Element', 'x', 'y', 'z'] in **fractional** coordinates.
        CellDim : pandas.DataFrame
            Shape (3, 3) with columns ['x', 'y', 'z'] giving the three lattice vectors (in Å).
        Velocities : pandas.DataFrame, optional
            Returned only if `GiveVelocities=True` **and** velocities are present in the file.
            Shape (N, 4) with columns ['Element', 'vx', 'vy', 'vz'].
            The 'Element' order matches `Position`. No unit conversion is performed.
    ...
    ValueError
        If the file is malformed (e.g., unexpected EOF while reading positions).

    Notes
    -----
    - Selective dynamics flags are ignored when parsing positions (only first three numeric fields are read).
    - Velocity block, when present, is expected to have exactly N lines with 3 numeric values each.
    """
    ...
    if len(Vels) == NAtoms:
        ...
        # 🔹 Ensure element labels accompany velocities (aligned with Position order)
...
def WriteXYZ(
    ...
    if AppendMode:
        try:
            FileSize = os.path.getsize(FilePathStr)
            with open(FilePathStr, "rb") as FileHandle:
                SeekPos = max(0, FileSize - TailBytes)
            ...
            if StepMatches:
                StepOffset = int(StepMatches[-1].group(1))
            if TimeMatches:
